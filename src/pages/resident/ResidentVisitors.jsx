import React, { useMemo } from 'react';
    ...
    import { UserPlus, PlusCircle, Search, Archive as ArchiveIcon } from 'lucide-react';
    import { motion } from 'framer-motion';
    import ResidentVisitorTable from '@/components/resident/visitors/ResidentVisitorTable';
    import ResidentVisitorDialogs from '@/components/resident/visitors/ResidentVisitorDialogs';
    import { useResidentVisitorManagement } from '@/hooks/useResidentVisitorManagement';
    ...
    const ResidentVisitors = () => {
      ...
      if (loading) {
        return <Layout role="resident"><div className="flex items-center justify-center h-full dark:text-gray-300"><PlusCircle className="h-8 w-8 animate-spin mr-2" /> Loading your visitor data...</div></Layout>;
      }
      if (!currentUser) {
        return <Layout role="resident"><div className="text-center p-8 dark:text-gray-300">Could not load resident information. Please log in again.</div></Layout>;
      }
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div>
                <h1 className="text-3xl font-bold tracking-tight text-gray-800 dark:text-white flex items-center">
                  <UserPlus className="mr-3 h-8 w-8 text-primary" /> My Visitors
                </h1>
                <p className="text-muted-foreground">Manage guest access for your unit: <span className="font-semibold text-primary">{currentUser.unitNumber}</span></p>
              </div>
              <Button onClick={openAddVisitorDialog} className="bg-primary hover:bg-primary/90 text-primary-foreground">
                <PlusCircle className="mr-2 h-4 w-4" /> Invite New Visitor
              </Button>
            </div>
